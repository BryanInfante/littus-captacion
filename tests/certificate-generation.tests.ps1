...
Assert-NotContains $issueFunction "npm:pdf-lib" "La función no debe reconstruir el certificado con pdf-lib; el HTML es la fuente de verdad."
Assert-NotContains $issueFunction 'renderCertificatePdf' "La función no debe renderizar un PDF manual distinto al HTML aprobado."
Assert-NotContains $issueFunction 'uploadCertificateToNextcloud' "La función no debe subir un PDF visualmente distinto al HTML aprobado."
Assert-Contains $issueFunction 'buildPrintableCertificateUrl' "La función debe enviar enlace al certificado HTML imprimible."
Assert-Contains $issueFunction '/certificado.html?code=' "El enlace del certificado debe apuntar al HTML imprimible."
Assert-Contains $issueFunction 'sendCertificateEmail' "La función debe enviar correo con el enlace al certificado HTML."
Assert-Contains $issueFunction 'Idempotency-Key' "El correo debe usar idempotency key."
Assert-Contains $issueFunction 'response.status === 409' "Resend 409 por idempotency key repetida no debe invalidar un certificado HTML ya preparado."
...
Assert-Contains $issueFunction 'public_certificate_url' "La función debe persistir el enlace público del certificado HTML imprimible."
Assert-NotContains $issueFunction '/functions/v1/download-certificate?code=' "El enlace de certificado nuevo no debe apuntar al proxy PDF; debe apuntar al HTML imprimible."
...
Assert-Contains $certificateTemplate 'validate-certificate?code=' "El certificado HTML debe cargar datos reales por código."
Assert-Contains $certificateTemplate 'window.print()' "El certificado HTML debe permitir imprimir/guardar como PDF desde el navegador."
Assert-Contains $certificateTemplate 'api.qrserver.com' "El certificado HTML debe generar QR visible para impresión."
