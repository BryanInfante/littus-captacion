...
Assert-Contains $issueFunction "npm:@pdf-lib/fontkit" "La función debe registrar fuentes personalizadas con la build de fontkit compatible con Edge Functions (el paquete 'fontkit' base usa fs.readFileSync y no arranca en Deno)."
Assert-Contains $issueFunction "fonts.gstatic.com/s/spacegrotesk" "Los títulos del PDF deben usar la instancia estática real de Space Grotesk servida por Google Fonts (SpaceGrotesk-Bold.ttf no existe en el repo de Google Fonts y devuelve 404)."
Assert-Contains $issueFunction "fonts.gstatic.com/s/inter" "El cuerpo del PDF debe usar Inter como en la plantilla HTML, desde una instancia estática (no el variable font) para evitar el bug de subsetting."
Assert-Contains $issueFunction "subset: false" "Inter no debe subsettearse: pdf-lib/@pdf-lib/fontkit pierde la mayoría de sus glyphs al subsettear ese archivo."
Assert-Contains $issueFunction "npm:qrcode" "La función debe generar QR con librería edge-compatible."
Assert-Contains $issueFunction 'buildCertificateTemplate' "La función debe tener seam de construcción de la plantilla estática del PDF."
Assert-Contains $issueFunction 'stampCertificate' "La función debe tener seam de estampado dinámico (nombre, QR, código) sobre la plantilla."
...
Assert-Contains $issueFunction 'const buildLayout = () => ({' "El renderer PDF debe usar una matriz de layout equivalente al HTML."
...
Assert-Contains $issueFunction 'rightX: layout.pageWidth - layout.contentX' "El rótulo superior del QR debe alinearse con el margen de contenido (22mm), no quedar pegado al borde de la página."
...
Assert-Contains $issueFunction 'borderColor: cyan' "La onda del estampe debe usar el acento cyan de marca, no un color plano casi invisible."
Assert-Contains $issueFunction 'borderOpacity: 0.35' "La onda del estampe debe tener opacidad explícita, como el acento del HTML."
...
Assert-Contains $issueFunction 'logo-dark.png' "El correo del certificado debe usar el mismo header con logo que los demás correos operativos ECCIA."
Assert-Contains $issueFunction '#0A0E13' "El correo del certificado debe usar el mismo header/footer oscuro de marca que los demás correos ECCIA."
Assert-Contains $issueFunction 'const escapeHtml' "El correo del certificado debe escapar el nombre del asistente antes de insertarlo en el HTML."
Assert-Contains $issueFunction 'Gracias por' "El correo del certificado debe incluir un mensaje de agradecimiento por participar."
Assert-NotContains $issueFunction 'descuento' "El correo del certificado es operativo y no debe incluir contenido promocional de otros cursos."
...
Assert-Contains $validateFunction 'eccia_eventos' "La validación debe leer los metadatos del curso desde eccia_eventos."
Assert-NotContains $validateFunction "'Seminario de Ultrasonido Industrial Nivel I'" "validate-certificate no debe traer el título de ningún curso: devolvería ese título para certificados de cualquier otro curso."
Assert-Contains $validateFunction "'external'" "La validación debe aceptar certificados con estado external (emitidos fuera de la plataforma, sin PDF en Nextcloud)."
...
Assert-Contains $validationPage 'data.event_title' "La validación pública debe mostrar el curso que devuelve la función, no uno fijo."
